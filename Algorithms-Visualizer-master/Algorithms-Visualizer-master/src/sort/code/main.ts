...
const REAL_BOOK_NAMES = [
  "Hamlet", "Macbeth", "Othello", "Odyssey", "Dracula", "Hobbit",
  "Ulysses", "Inferno", "Emma", "Rebecca", "Ivanhoe", "Gulliver",
  "Beloved", "Lolita", "Candide", "Beowulf", "Wuthering", "Kidnapped",
  "Babbitt", "Pinocchio", "Peterpan", "Aeneid", "Walden", "Zadig",
  "Frankenstein", "Persuasion", "Middlemarch", "Silmarillion", "Jungle", "Kim",
  "Sidereus", "Metamorphosis", "Eneida", "Faust", "Gitanjali", "Heidi",
  "Iliad", "Janeeyre", "Lesmis", "Mobydick", "Narnia", "Oliver",
  "Paradise", "Quixote", "Robinson", "Siddhartha", "Tempest", "Utopia",
  "Vanity", "Warpeace", "Xenophon", "Youth", "Zorba"
];
...
function createBookshelf() {
  ...
  // Shuffle the book names
  const shuffled = [...REAL_BOOK_NAMES].sort(() => Math.random() - 0.5);
  
  for (let i = 0; i < books; i++) {
    const bookName = shuffled[i % shuffled.length];
    bookshelf.push(new Book(bookName));
  }
...
function createWorstBookshelf() {
  ...
  // Get a random subset of book names of size `books`
  const shuffled = [...REAL_BOOK_NAMES].sort(() => Math.random() - 0.5);
  const subset: string[] = [];
  for (let i = 0; i < books; i++) {
  ...
  // Sort the subset in reverse alphabetical order (Z to A)
  subset.sort((a, b) => b.localeCompare(a));
  
  for (let i = 0; i < books; i++) {
    bookshelf.push(new Book(subset[i]));
  }
  bookshelfStack = [...bookshelf]; // save the original array to the stack
  visualizeBookshelf();
}
